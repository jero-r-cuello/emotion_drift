...
possible_verbosity_levels = ["low", "medium", "high"]
possible_effort_levels = ["minimal", "low", "medium", "high"]

parameters_combinations = [
    (verbosity_level, effort_level)
...
for model_name in models_names:
    ...
    main_iterator = itertools.product(
        taxonomies_to_use,
        # Puesto para una cuarta run
    enumerate(generated_responses[3 * chunk_size:], start=3 * chunk_size)
    )

    for taxonomy, (i, response_text) in main_iterator:
        emotions_definition = definitions_of_emotions[taxonomy]
        instruction_prompt = create_annotation_prompt(response_text, emotions_definition)

        for verbosity_level, effort_level in parameters_combinations:
            request = {
                "custom_id": f"request-{i}-{model_name}-{taxonomy}-{verbosity_level}-{effort_level}",
                "method": "POST",
                "url": "/v1/responses",
                "body": {
                    "model": model_name,
                    ...
                    "prompt_cache_key": "gen-1757602875-878DOlG67xz757xY9tkV" # Cache key from anothre run, maybe I should use a different one?
                }
            }
            batch_requests.append(request)

    # Crear archivo JSONL para este modelo
    path_jsonl = f"batch_annotation_jobs_{model_name}.jsonl"
    with open(path_jsonl, "w") as f:
        for req in batch_requests:
            f.write(json.dumps(req) + "\n")

    print(f"Total requests for {model_name}: {len(batch_requests)}")
    print(f"Saved file: {path_jsonl}")
    ...
    client.batches.create(
        ...
        metadata={
            "description": f"Batch for testing {model_name} with verbosity/effort parameters."
        }
    )
# %%
