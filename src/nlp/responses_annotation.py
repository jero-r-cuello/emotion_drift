...
OUTPUT_FILE_PATH = f"{BASE_DIR}/data/04_annotated/batch_results_{RUN_TO_PROCESS}.jsonl"
...
def monitor_batch(batch_id: str):
    """Revisa el estado con el backoff strategy definido."""
    ...
    while True:
        ...
        print(f"   Esperando {sleep_time} segundos para la próxima revisión...")
        time.sleep(sleep_time)

# ================= PROCESO PRINCIPAL =================

def main():
    # 1. Crear el archivo de salida (si no existe) o asegurar que esté listo para append
    if not os.path.exists(OUTPUT_FILE_PATH):
        print(f"Creando archivo de resultados: {OUTPUT_FILE_PATH}")
        with open(OUTPUT_FILE_PATH, 'w', encoding='utf-8') as f:
            pass 

    # 2. Leer datos de entrada
    print(f"Leyendo archivo de entrada: {INPUT_FILE_PATH}")
    input_data = []
    ...
    for i in tqdm(range(0, total_texts, BATCH_SIZE), desc="Procesando Batches", unit="batch"):
        chunk = input_data[i : i + BATCH_SIZE]
        print(f"\n--- Procesando Batch de Textos {i} a {min(i + BATCH_SIZE, total_texts)} ---")
        ...
        try:
            # Subir archivo
            print(f"Subiendo archivo {temp_input_filename}...")
            ...
            print(f"Batch Job creado con ID: {batch_job.id}")
            ...
            if finished_batch.status == 'completed' and finished_batch.output_file_id:
                ...
                # Las líneas tal cual vienen de OpenAI
                result_lines = content.strip().split('\n')
                
                # Guardar (Append) al archivo principal
                save_lines_to_file(OUTPUT_FILE_PATH, result_lines)
                print(f"Resultados guardados ({len(result_lines)} líneas) en {OUTPUT_FILE_PATH}")
                
            # MODIFICACIÓN: Chequeo específico de status 'failed' y descarga del error
            elif finished_batch.status == 'failed':
                print(f"¡El Batch falló (Status: failed)!")
                ...
                # Caso 2: Fallo con archivo generado (menos común en validación, pero posible)
                    ...
                    print(f"Descargando archivo de errores (ID: {finished_batch.error_file_id})...")
                    try:
                    ...
                    print("El batch falló pero no hay detalles de 'errors' ni 'error_file_id'.")

            # Otros estados (expired, cancelled, etc.)
            else:
                print(f"El batch terminó con estado: {finished_batch.status}")
                if finished_batch.error_file_id:
                    print(f"Existe un archivo de errores (ID: {finished_batch.error_file_id}), pero el status no es 'failed' o 'completed'.")

        except Exception as e:
            print(f"Ocurrió un error procesando el chunk {i}: {e}")
