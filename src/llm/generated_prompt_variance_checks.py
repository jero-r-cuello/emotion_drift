...
def preprocessing(file_name, n_prompts_expected, save):
    if file_name.endswith(".txt"):
        ...
        print("Checking the length of prompt lists")
        for emotion, prompts in prompts_per_emotion_dict.items():
            current_len = len(prompts)
            print(f"Emotion: {emotion}, Length: {current_len}")
        ...
        if excluded_emotions:
            print("\n!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
            print("WARNING: The following emotions were excluded because they did not have the expected length of prompts:")
            for emotion, length in excluded_emotions:
                print(f"- '{emotion}': {length} prompts were found instead of the expected {n_prompts_expected}.")
            print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n")
        ...
        if save:
            try:
                ...
                print(f"An unexpected error occurred while creating or saving the DataFrame: {e}")
...
def analisis_sintactico(lista_oraciones, n_of_generation, save):
    ...
    df_longitudes = pd.DataFrame(longitudes, columns=["length"])
    
    print("\nLength of sentences stats:")
    print(df_longitudes.describe())

    # 1. Length distribution
    plt.figure(figsize=(10, 6))
    sns.histplot(data=df_longitudes, x="length", kde=True, bins=25)
    plt.title(f'Distribution of length of sentences (generated prompts per emotion by each model = {n_of_generation})')
    ...
    df_inicio = pd.DataFrame(conteo_inicio, columns=["word", "frequency"])
    
    plt.figure(figsize=(12, 7))
    sns.barplot(data=df_inicio, x="frequency", y="word", palette="tab10")
...
def semantic_analysis(lista_oraciones, n_of_generation, save):
    ...
    df_plot["sentence"] = lista_oraciones
