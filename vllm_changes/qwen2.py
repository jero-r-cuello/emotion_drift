...
from vllm.hook_store import ACTIVATION_HOOKS  # <--- ADD THIS
...
def _save_activations_in_batches(gpu_buffer, save_path, prompt_key, batch_size):
    ...
    for i, (layer_idx, tensor_gpu) in enumerate(gpu_buffer):
        # 1. Move tensor to CPU and add to buffer
        cpu_buffer.append((layer_idx, tensor_gpu.cpu()))

        is_last_item = (i + 1) == len(gpu_buffer)
        # 2. Check if buffer is full or it is the last item
...
class Qwen2Model(nn.Module):
    ...
    def forward(
        ...
        # Check if hooks are active (and if we are in the "prefill/prompt" phase, usually indicated by shape > 1)
        ...
        # Note: Added enumerate() to get the layer index
        for idx, layer in enumerate(self.layers[self.start_layer:self.end_layer]):
            
            # 1. Capture Logic
            if hooks_are_active and idx in target_layers:
                # Capture the hidden state BEFORE the layer processes it
